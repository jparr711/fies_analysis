...
ml_hh_baseline_import <- read_dta("FTF_ZOI_2019_Mali_Household_NonPublic.dta")

# OR
# setwd("C:/Users/N116937/OneDrive - ICF/Documents/Data Analysis/Projects/FIES")
ml_hh_baseline_import <- read_dta("Mali ZOI Survey 2019 household data analytic RS.dta")

# READ MIDLINE
# setwd("C:/Users/N116937/ICF/SMRFS - 14. Mali Midline/08. Data Analysis/Midline/Processed_data/Analytic")
ml_hh_midline_import <- read_dta("FTF_P2-ZOI_2023_Mali_Survey_household_data_analytic.dta")


# setwd("C:/Users/N116937/OneDrive - ICF/Documents/Data Analysis/Projects/FIES/Mali_FIES")
# varlist
# vtable(ml_hh_midline_import)
# vtable(ml_hh_baseline_import)
...
# 3 cases with all items missing, all respondent not at home. remove
...
ml_hh_baseline_import %>%
  ...
  dplyr::filter(!complete.cases(.)) %>%
  as.data.frame()
# 31 rows with incomplete cases
# first ten have v300d codes, otherwise NA - refers to HHs who did not participate in interview as a whole

# distribution of v300d
ml_hh_baseline_import %>%
  dplyr::count(v300d)
# 3 CONSENT REFUSED, 7 RESPONDENT NOT AVAILABLE
...
# Incomplete cases - all have NAs for all FIES questions. Includes 9 completed interviews, the rest are incomplete for various reasons (as shown in previous table)
...
ml_hh_baseline <- ml_hh_baseline_import

ml_hh_bl_shock_sev <- read_dta("FTF_P2-ZOI_Survey_Mali_2019_resilience.dta")


ml_hh_midline  <- ml_hh_midline_import

ml_hh_ml_wealth <- read_dta("FTF_P2-ZOI_2023_Mali_Survey_wealthindex_AWI_recalc.dta")
ml_hh_ml_shock_sev <- read_dta("FTF_P2-ZOI_Survey_Mali_2023_resilience.dta")
...
ml_hh_baseline <- 
  ml_hh_baseline %>% mutate_at(dplyr::vars(v301:v308),
...
table(is.na(ml_hh_baseline$genhhtype_dj))

table(ml_hh_baseline$genhhtype_dj)

ml_hh_baseline$genhhtype_dj <- factor(ml_hh_baseline$genhhtype_dj, levels = c(1,2,3,4), labels = c("De jure male and female adults","De jure female, no male", 
                                                                                                 "De jure male, no female", "De jure children only")) 



# cluster and HH id
ml_hh_baseline$hhea <- ml_hh_baseline$hhea
# hhnum
ml_hh_baseline$hhnum <- ml_hh_baseline$hhnum
# psu
ml_hh_baseline$psu <- ml_hh_baseline$a03d
# NEED TO RECODE - MORE UNITS THAN IN BL (6 regions vs. 3)
ml_hh_baseline$psu <- factor(ml_hh_baseline$psu, levels=c(1,2,3), labels=c("Bougouni", "Koutiala", "Sikasso"))

# Add cercle - 
ml_hh_baseline %>%
  dplyr::count(a03c)

ml_hh_baseline$cercle <- factor(ml_hh_baseline$a03c, 
                                levels=c(13,14,15,16,17,18), 
                                labels=c("Bougouni", "Kadiolo", "Koutiala", 
                                         "Sikasso", "Yanfolila", "Yorosso"))

ml_hh_baseline %>%
  dplyr::count(cercle)

# create combined weight and HH size
ml_hh_baseline$wt <- ml_hh_baseline$wgt_hh
ml_hh_baseline$wt_mem <- ml_hh_baseline$wt * ml_hh_baseline$hhsize_dj

ml_hh_baseline$ahtype <- factor(ml_hh_baseline$ahtype, levels=c(1,2), labels=c("Urban","Rural"))

# strata
ml_hh_baseline$strata <- ml_hh_baseline$a03c
# survey round- midline == 2
ml_hh_baseline$survey <- 1



# wealth quintile
table(ml_hh_baseline$awiquint)
table(ml_hh_baseline$awiquint_rev)
table(is.na(ml_hh_baseline$awiquint))
# ml_hh_baseline$awiquint_rev <- forcats::fct_rev(factor(as.character(ml_hh_baseline$awiquint)))
ml_hh_baseline$awiquint_rev <- factor(ml_hh_baseline$awiquint, levels = c(1,2,3,4,5), labels = c("Poorest","Second", "Middle", "Fourth", "Wealthiest"))
...
ml_hh_baseline <- 
  ...
  left_join(ml_hh_bl_shock_sev %>% dplyr::select(hhea, hhnum, shock_sev))

ml_hh_baseline$shock_sev <- factor(ml_hh_baseline$shock_sev, levels = c(1,2,3,4), labels = c("None", "Low", "Medium", "High"))


# edu level


ml_hh_baseline$edulevel_hh_dj <- factor(ml_hh_baseline$edulevel_hh2_dj, levels = c(1,2,3,4,5,6), labels = c("No education","Primary One 1-3", "Primary One 4-6", 
...
head(ml_hh_baseline$hhid)

# select vars to keep, drop vars not needed for analysis

ml_hh_baseline <- 
  ml_hh_baseline %>%
  dplyr::select(survey, hhid, hhea, hhnum, psu, cercle, wt, hhsize_dj, wt_mem, strata, genhhtype_dj, ahtype, 
                edulevel_hh_dj, 
...
# recode FIES vars - NO response from 2 to 0
ml_hh_midline <- 
  ml_hh_midline %>% mutate_at(dplyr::vars(v301:v308),
                              dplyr::funs(case_when(
                                . == 1 ~ 1,
                                . == 2 ~ 0)
                              ))
...
# individual item missing values are on 1 item (ATELESS) where pattern is clear (all 0's). can be safely imputed to 0.
# 2 HH in baseline, 1 HH in midline with this pattern.

ml_hh_baseline <-
  ml_hh_baseline %>%
  dplyr::mutate(RUNOUT = case_when(WORRIED == 0 & HEALTHY == 0 & FEWFOOD == 0 & SKIPPED == 0 & 
                                      ATELESS  == 0 & HUNGRY  == 0 & WHLDAY  == 0 & is.na(RUNOUT) ~ 0,
...
ml_hh_midline$wt <- ml_hh_midline$wgt_hh


# cluster and HH id
ml_hh_midline$hhea <- ml_hh_midline$hhea
# hhnum
ml_hh_midline$hhnum <- ml_hh_midline$hhnum
# psu
ml_hh_midline$psu <- ml_hh_midline$c05
# NEED TO RECODE - MORE UNITS THAN IN BL (6 regions vs. 3)
ml_hh_midline$psu <- factor(ml_hh_midline$psu, levels=c(1,2,3,4, 5, 6), labels=c("1", "2", "3", "4", "5", "6"))
...
ml_hh_midline$cercle <- factor(ml_hh_midline$c05, 
                                levels=c(1,2,3,4,5,6), 
                                labels=c("Sikasso", "Bougouni", "Kadiolo", 
                                         "Koutiala", "Yanfolila", "Yorosso"))
...
# create combined weight and HH size
ml_hh_midline$wt_mem <- ml_hh_midline$wt * ml_hh_midline$hhsize_dj


# strata
ml_hh_midline$strata <- ml_hh_midline$stratum
# survey round- midline == 2
ml_hh_midline$survey <- 2

# add background variables as factors, with levels and labels
# HH type
ml_hh_midline$genhhtype_dj <- ml_hh_midline$genhhtype_dj
# NOT IN DATASET
ml_hh_midline$genhhtype_dj <- factor(ml_hh_midline$genhhtype_dj, levels = c(1,2,3,4), labels = c("De jure male and female adults","De jure female, no male", 
                                                                                                  "De jure male, no female", "De jure children only")) 


# urban - rural
ml_hh_midline$ahtype <- factor(ml_hh_midline$ahtype, levels=c(1,2), labels=c("Urban","Rural"))
# highest HH education level
# NOT IN DATASET
ml_hh_midline$edulevel_hh_dj <- factor(ml_hh_midline$edulevel_hh2_dj, levels = c(1,2,3,4,5,6), labels = c("No education","Primary One 1-3", "Primary One 4-6", 
                                                                                                            "Primary 2 1-3", "Secondary 1-4",
                                                                                                            "Higher"))
# wealth quintile
# NOT IN DATASET. JOIN OTHRE DATASET TO ADD?

names(ml_hh_ml_wealth)
ml_hh_ml_wealth <- ml_hh_ml_wealth %>%
  dplyr::select(hhea, hhnum, awi, awiquint)

ml_hh_midline <- 
  ml_hh_midline %>%
  dplyr::select(-awi, -awiquint) %>%
  left_join(ml_hh_ml_wealth)

ml_hh_midline$awiquint_rev <- factor(ml_hh_midline$awiquint, levels = c(1,2,3,4,5), labels = c("Poorest","Second", "Middle", "Fourth", "Wealthiest"))

ml_hh_midline$awiquint
ml_hh_midline$awiquint_rev
table(ml_hh_midline$awiquint_rev)
table(ml_hh_midline$awiquint)



table(ml_hh_ml_wealth$awiquint)
table(ml_hh_midline$awiquint)
# 
# table(ml_hh_midline$awiquint)
# 
# ml_hh_midline$awiquint_rev <- forcats::fct_rev(factor(as.character(ml_hh_midline$awiquint), 
#                                                       levels = c(1,2,3,4,5), 
...
ml_hh_midline <- 
  ml_hh_midline %>%
  dplyr::select(-shock_sev) %>%
   left_join(ml_hh_ml_shock_sev %>% dplyr::select(hhea, hhnum, shock_sev))

head(ml_hh_midline$shock_sev)
# 
ml_hh_midline$shock_sev <- factor(ml_hh_midline$shock_sev, levels = c(1,2,3,4), labels = c("None", "Low", "Medium", "High"))
table(ml_hh_midline$shock_sev)
# No HHID?
ml_hh_midline <- 
  ml_hh_midline %>%
  mutate(hhid = row_number())
...
ml_hh_midline <- 
  ml_hh_midline %>%
  dplyr::select(survey, hhid, hhea, hhnum, wt, psu, cercle, strata, ahtype,
                hhsize_dj, wt_mem,  
...
ml_hh_combined <- bind_rows(ml_hh_baseline, ml_hh_midline)
...
ml_hh_baseline$RS = rowSums(FIES_baseline)
ml_hh_midline$RS = rowSums(FIES_midline)
...
# identify items in common - one where scores are similar. all scores are similar in MALI except HUNGRY (7) and FEWFOODS (3)
common = colnames(FIES_baseline[, c(-3, -7)])
...
RS_table = t(cbind(
  "Baseline" = aggregate(ml_hh_baseline$wt, list(ml_hh_baseline$RS), FUN=sum, na.rm=TRUE)$x /sum(ml_hh_baseline$wt[!is.na(ml_hh_baseline$RS)]),
  "Midline" = aggregate(ml_hh_midline$wt, list(ml_hh_midline$RS), FUN=sum, na.rm=TRUE)$x /sum(ml_hh_midline$wt[!is.na(ml_hh_midline$RS)])
))
...
# columns 1-6
glob_st.m <- mean(glob_st[c(1:6)])
glob_st.s <- sd.p(glob_st[c(1:6)])
m.bl = mean(res_bl$b[c(1:6)]) 
s.bl = sd.p(res_bl$b[c(1:6)])
...
ml_hh_baseline$prob_mod_sev = NULL
ml_hh_midline$prob_mod_sev = NULL
ml_hh_baseline$prob_mod = NULL
ml_hh_midline$prob_mod = NULL
ml_hh_baseline$prob_sev = NULL
ml_hh_midline$prob_sev = NULL


for (rs in 0:8) {
  ml_hh_baseline$prob_mod[ml_hh_baseline$RS == rs] = glo_probs_bl_mod[rs+1]
  ml_hh_midline$prob_mod[ml_hh_midline$RS == rs] =   glo_probs_ml_mod[rs+1]
  ml_hh_baseline$prob_mod_sev[ml_hh_baseline$RS == rs] = glo_probs_bl_mod_sev[rs+1]
  ml_hh_midline$prob_mod_sev[ml_hh_midline$RS == rs] =   glo_probs_ml_mod_sev[rs+1]
  ml_hh_baseline$prob_sev[ml_hh_baseline$RS == rs] = glo_probs_bl_sev[rs+1]
  ml_hh_midline$prob_sev[ml_hh_midline$RS == rs] =   glo_probs_ml_sev[rs+1]
}

table(ml_hh_baseline$prob_mod,ml_hh_baseline$RS,useNA = "ifany")

table(ml_hh_baseline$prob_mod_sev,ml_hh_baseline$RS,useNA = "ifany")

table(ml_hh_baseline$prob_sev,ml_hh_baseline$RS,useNA = "ifany")


table(ml_hh_midline$prob_mod,ml_hh_midline$RS,useNA = "ifany")

table(ml_hh_midline$prob_mod_sev,ml_hh_midline$RS,useNA = "ifany")

table(ml_hh_midline$prob_sev,ml_hh_midline$RS,useNA = "ifany")
...
modsev_moe_bl_95 <- moe(ml_hh_baseline$prob_mod_sev,ml_hh_baseline$RS,ml_hh_baseline$wt * 10^6, conf.level = .95,
                        psu = ml_hh_baseline$psu, strata = ml_hh_baseline$strata)$moe * 100
mod_moe_bl_95 <- moe(ml_hh_baseline$prob_mod,ml_hh_baseline$RS,ml_hh_baseline$wt * 10^6, conf.level = .95,
                     psu = ml_hh_baseline$psu, strata = ml_hh_baseline$strata)$moe * 100
sev_moe_bl_95 <- moe(ml_hh_baseline$prob_sev,ml_hh_baseline$RS,ml_hh_baseline$wt * 10^6, conf.level = .95,
                     psu = ml_hh_baseline$psu, strata = ml_hh_baseline$strata)$moe * 100

moe_bl_95 <- c(modsev_moe_bl_95, mod_moe_bl_95, sev_moe_bl_95)

modsev_moe_ml_95 <- moe(ml_hh_midline$prob_mod_sev,ml_hh_midline$RS,ml_hh_midline$wt * 10^6, conf.level = .95,
                        psu = ml_hh_midline$psu, strata = ml_hh_midline$strata)$moe * 100
mod_moe_ml_95 <- moe(ml_hh_midline$prob_mod,ml_hh_midline$RS,ml_hh_midline$wt * 10^6, conf.level = .95,
                     psu = ml_hh_midline$psu, strata = ml_hh_midline$strata)$moe * 100
sev_moe_ml_95 <- moe(ml_hh_midline$prob_sev,ml_hh_midline$RS,ml_hh_midline$wt * 10^6, conf.level = .95,
                     psu = ml_hh_midline$psu, strata = ml_hh_midline$strata)$moe * 100
...
AGG_df[1, c(5)] <- round(sum(ml_hh_baseline$wt * 10^6), 2)
...
AGG_df[2, c(5)] <- round(sum(ml_hh_midline$wt * 10^6), 2)
...
group1 = ml_hh_combined$survey
group1 <- factor(group1, levels = c(1,2), labels = c("Baseline","Midline")) 
# URBAN/RURAL
group2 = ml_hh_combined$ahtype
# REGION
group3 = ml_hh_combined$cercle
# GEN HH TYPE
group4 = ml_hh_combined$genhhtype_dj
# SHOCK SEV
group5 <- ml_hh_combined$shock_sev
# AWIQUINT
group6 <- ml_hh_combined$awiquint_rev
# EDUCATION
group7 <- as.character(ml_hh_combined$edulevel_hh_dj)

group_list <- list(group2, group3, group4, group5, group6, group7)
...
for (i in 1:length(group_list)) {
  for (round in unique(group1)) {
    for (dis in unique(groups)) {
      ...
      prob_mods = ml_hh_combined$prob_mod[fltr]
      wt = ml_hh_combined$wt[fltr]*10^6
      rs = ml_hh_combined$RS[fltr]
      psu = ml_hh_combined$psu[fltr] 
      strata = ml_hh_combined$strata[fltr]
      output_1 = rd 
      output_2 = disag
...
for (i in 1:length(group_list)) {
  for (round in unique(group1)) {
    for (dis in unique(groups)) {
      ...
      disag = dis
      fltr = which(group1 == round &  group_list[[i]] == dis)
      prob_mods = ml_hh_combined$prob_sev[fltr]
      wt = ml_hh_combined$wt[fltr]*10^6
      rs = ml_hh_combined$RS[fltr]
      psu = ml_hh_combined$psu[fltr] 
      strata = ml_hh_combined$strata[fltr]
...
for (i in 1:length(group_list)) {
  for (round in unique(group1)) {
    for (dis in unique(groups)) {
      ...
      disag = dis
      fltr = which(group1 == round &  group_list[[i]] == dis)
      prob_mods = ml_hh_combined$prob_mod_sev[fltr]
      wt = ml_hh_combined$wt[fltr]*10^6
      rs = ml_hh_combined$RS[fltr]
      psu = ml_hh_combined$psu[fltr] 
      strata = ml_hh_combined$strata[fltr]
...
table_5_1_1 <-
  ...
  arrange(match(Disaggregate, c("De jure male and female adults", "De jure female, no male", 
                                ...
                                "Bougouni", "Kadiolo", "Koutiala", "Sikasso", "Yanfolila",
                                "Yorosso", 
                                "No education", "Primary One 1-3",
                                "Primary One 4-6", "Primary 2 1-3")))
...
table_5_1_2 <-
  ...
  arrange(match(Disaggregate, c("De jure male and female adults", "De jure female, no male", 
                                ...
                                "Wealthiest", "Fourth", "Middle", "Second", "Poorest",
                                "Bougouni", "Kadiolo", "Koutiala", "Sikasso", "Yanfolila",
                                "Yorosso", "Urban", "Rural", "None", "Low", "Medium", "High",
                                "No education", "Primary One 1-3",
                                "Primary One 4-6", "Primary 2 1-3"))) %>%
...
table_5_1_1 <- bind_rows(AGG_df_511_join, table_5_1_1[1:20,])
table_5_1_2 <- bind_rows(AGG_df_512_join, table_5_1_2[1:20,])
...
# setwd("C:/Users/N116937/ICF/SMRFS - 14. Mali Midline/08. Data Analysis/Midline/X2. Jamie/Results")
setwd("C:/Users/jparr/OneDrive/Documents/ICF/Analysis/Mali/R/FIES")
write_csv(table_5_1_1, "table_5_1_1_upd_01-16-2024.csv")
write_csv(table_5_1_2, "table_5_1_2_upd_01-16-2024.csv")

write_dta(ml_hh_combined, "FTF_P2-ZOI_Mali_Survey_BL_ML-FIES.dta")


library(survey)
tvar <- moe(ml_hh_midline$prob_mod_sev,ml_hh_midline$RS,ml_hh_midline$wt, conf.level = .95,
    psu = ml_hh_midline$psu, 
    strata = ml_hh_midline$strata)

tvar$deff_s

tvarsdes <- svydesign(id=ml_hh_midline$psu[ml_hh_midline$genhhtype_dj == "De jure male and female adults"],
                      weights = ~ml_hh_midline$wt[ml_hh_midline$genhhtype_dj == "De jure male and female adults"],
                      strata=ml_hh_midline$strata[ml_hh_midline$genhhtype_dj == "De jure male and female adults"],
                      nest=T,
                      lonely.psu=getOption("survey.lonely.psu"="adjust"))

survey:::deff(svymean(~ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure male and female adults"], 
                      tvarsdes, 
                      deff=T, na.rm =F))
...
ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure male and female adults"]

tvarsdes <- svydesign(id=ml_hh_midline$psu,weights = ~ml_hh_midline$wt,
                      strata=ml_hh_midline$strata,
                      nest=T,
                      lonely.psu=getOption("survey.lonely.psu"="adjust"))
...
moe(ml_hh_midline$prob_mod_sev,
    ml_hh_midline$RS,
    ml_hh_midline$wt * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea, 
    strata = ml_hh_midline$strata)

moe(ml_hh_midline$prob_mod,
    rs = ml_hh_midline$RS,
    wt = ml_hh_midline$wt * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea, 
    strata = ml_hh_midline$strata)

moe(ml_hh_midline$prob_sev,
    ml_hh_midline$RS,
    ml_hh_midline$wt * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea, 
...
moe(ml_hh_midline$prob_mod,
    ...
    conf.level = .95,
    psu = ml_hh_midline$psu, 
    strata = ml_hh_midline$strata)$se_s



moe(ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure male and female adults"],
    ml_hh_midline$RS[ml_hh_midline$genhhtype_dj == "De jure male and female adults"],
    ml_hh_midline$wt[ml_hh_midline$genhhtype_dj == "De jure male and female adults"] * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea[ml_hh_midline$genhhtype_dj == "De jure male and female adults"], 
    strata = ml_hh_midline$strata[ml_hh_midline$genhhtype_dj == "De jure male and female adults"])



moe(ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure female, no male"],
    ml_hh_midline$RS[ml_hh_midline$genhhtype_dj == "De jure female, no male"],
    ml_hh_midline$wt[ml_hh_midline$genhhtype_dj == "De jure female, no male"] * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea[ml_hh_midline$genhhtype_dj == "De jure female, no male"], 
    strata = ml_hh_midline$strata[ml_hh_midline$genhhtype_dj == "De jure female, no male"])

moe(ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure male, no female"],
    ml_hh_midline$RS[ml_hh_midline$genhhtype_dj == "De jure male, no female"],
    ml_hh_midline$wt[ml_hh_midline$genhhtype_dj == "De jure male, no female"] * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$hhea[ml_hh_midline$genhhtype_dj == "De jure male, no female"], 
    strata = ml_hh_midline$strata[ml_hh_midline$genhhtype_dj == "De jure male, no female"])


sum(ml_hh_midline$wt)

ml_hh_midline$wt/max(ml_hh_midline$wt)
...
moe(ml_hh_midline$prob_mod_sev[ml_hh_midline$genhhtype_dj == "De jure male, no female"],
    ...
    ml_hh_midline$wt[ml_hh_midline$genhhtype_dj == "De jure male, no female"] * 10^6, 
    conf.level = .95,
    psu = ml_hh_midline$psu[ml_hh_midline$genhhtype_dj == "De jure male, no female"], 
    strata = ml_hh_midline$strata[ml_hh_midline$genhhtype_dj == "De jure male, no female"])$se_s
